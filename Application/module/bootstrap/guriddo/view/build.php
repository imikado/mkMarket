<?php if ($this->bEnableLoading): ?>
	...
	<link rel="stylesheet" type="text/css" href="guriddo/css/trirand/ui.jqgrid-bootstrap.css" media="screen" />
	...
	<script type="text/javascript">
		...
		$.jgrid.useJSON = true;
		$.jgrid.defaults.width = "<?php echo $this->iWidth ?>";</script>
<?php endif; ?>
...
<div>
	...
	<script type='text/javascript'>
		$(document).ready(function($) {$('#<?php echo $this->idTable ?>').jqGrid(
		{
		"styleUI" : 'Bootstrap',
...
<?php if ($this->bSortable): ?>
			"sortable": true,
<?php endif; ?>
<?php if ($this->bScrollbarPagingEnable): ?>
			"page": 1,
				"scroll":1,
<?php endif; ?>
		"colModel":[
...
<?php foreach ($this->tHeader as $tDetail): ?>
	...
	<?php foreach ($tDetail as $sKey => $sValue): ?>
				...
				'<?php echo $sKey ?>':<?php if (in_array($sKey, array('formatter', 'cellattr'))) {
			echo $sValue;
		} else {
...
<?php endforeach; ?>
			...
			try {
			$.jgrid.info_dialog($.jgrid.errors.errcap, '<div class="ui-state-error">' + xhr.responseText + '</div>', $.jgrid.edit.bClose, {buttonalign:'right'});
			} catch (e) {
...
<?php if ($this->tGroupHeader): ?>
			$('#<?php echo $this->idTable ?>').setGroupHeaders({
...
<?php endif; ?>
		...
		$('#<?php echo $this->idTable ?>').jqGrid('filterToolbar', {"stringResult":true});
